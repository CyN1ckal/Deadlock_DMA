...
void MainMenu::Render()
{
	...
	if (ImGui::Button("Restart DMA Reads", ImVec2(-1, 0)))
	{
		DMA_Connection* Conn = DMA_Connection::GetInstance();
		Deadlock::Restart(Conn);
	}
	if (ImGui::IsItemHovered())
	{
		ImGui::SetTooltip("Reinitialize all DMA reads (process, offsets, entities) without restarting the application");
	}
